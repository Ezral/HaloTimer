...
The separate test APK includes an original, clearly labelled sample recipe app. It is a backdrop for demonstrating real application overlays, not a Halo feature and not shipped in the app. The raw recording also visits Android Settings; that protected system screen hides overlays on this emulator, so the finished marketing edits exclude it. All countdowns, alerts, docks and menus are rendered by Halo's production implementation. No timer animation is composited onto the capture.
...
Reviewed the actual three-track overlays, both menu themes, sequence editor, dock menu and sampled video frames. Android Settings hides overlays in the captured system build; do not caption that screen as displaying a live overlay. The campaign renderer normalizes video timestamps and can hold the final frame briefly to reach an exact clip duration. It does not speed up the timer or synthesize its animations.
